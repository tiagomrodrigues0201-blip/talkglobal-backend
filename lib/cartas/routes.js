...
function createVerificarAuthCartas(supabase) {
  return async function verificarAuthCartas(req, res, next) {
    try {
      const token = getBearerToken(req);

      if (!token) {
        return sendError(res, 401, "token_not_sent", "Token não enviado.");
      }
      ...
      return sendError(res, 500, "cards_auth_failed", error.message || "Erro ao validar autenticação.");
    }
  };
}
...
function registerCartasRoutes(app, { supabase, verificarAuth = createVerificarAuthCartas(supabase) }) {
