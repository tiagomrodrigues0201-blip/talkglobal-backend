...
async function ensureProgression(supabase, userId) {
  ...
  if (error && error.code !== "23505") {
    throw new Error(`Erro ao preparar progressão: ${error.message}`);
  }
...
async function createInitialCards({ supabase, userId, photoProfile }) {
  ...
  const existingCards = await listPlayerCards(supabase, userId);
  const existingInitial = existingCards.filter((card) => card.isInitial);
  if (existingInitial.length >= 3) {
    return { created: false, reason: "initial_cards_already_created", cards: existingCards };
  }

  const { data: generationToday, error: generationError } = await supabase
  ...
  if (generationError) {
    throw new Error(`Erro ao consultar geração diária: ${generationError.message}`);
  }

  if (generationToday) {
    return { created: false, reason: "daily_free_generation_already_used", cards: existingCards };
  }

  const generationInsert = await supabase.from("card_generations").insert({
    user_id: userId,
    generation_date: today,
    kind: "initial",
    quantity: 3,
    metadata: { photoProfile },
    created_at: now
  });

  if (generationInsert.error) {
    if (generationInsert.error.code === "23505") {
      return { created: false, reason: "daily_free_generation_already_used", cards: existingCards };
    }
    throw new Error(`Erro ao registrar geração diária: ${generationInsert.error.message}`);
  }
  ...
  if (insertError) {
    if (insertError.code === "23505") {
      const cards = await listPlayerCards(supabase, userId);
      return { created: false, reason: "initial_cards_already_created", cards };
    }
    throw new Error(`Erro ao criar cartas iniciais: ${insertError.message}`);
  }

  const deckPayload = (inserted || []).map((card, index) => ({
    user_id: userId,
    card_id: card.id,
    slot: index + 1,
    updated_at: now
  ...
  if (deckPayload.length) {
    await supabase.from("player_decks").upsert(deckPayload, { onConflict: "user_id,slot" });
  }

  await ensureProgression(supabase, userId);
  return { created: true, reason: "initial_cards_created", cards: (inserted || []).map(normalizeCard) };
}
